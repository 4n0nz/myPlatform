...
type Props = {
  ...
  chatFontSize: 'S' | 'M' | 'L'
  blockUser: (uid: string, name: string) => void
}
...
export default function ChatPanel({
  ...
  onAuthRequired,
  isMuted, mutedUntil,
  chatFontSize, blockUser,
}: Props) {
  const msgPx = chatFontSize === 'S' ? 9 : chatFontSize === 'L' ? 13 : 11
  const iconPx = chatFontSize === 'S' ? 11 : chatFontSize === 'L' ? 15 : 13
  return (
    <div className='flex flex-col overflow-hidden relative p-3 gap-3' style={{ flex: '1 1 0' }}>
      ...
      <div className='border-b border-[#00ff41]/30 px-3 py-1.5 text-[10px] tracking-widest text-[#00ff41]/60 shrink-0 flex items-center justify-between'>
        ...
        <button
          onClick={() => window.open('/chat', 'chat', 'width=420,height=700,resizable=yes')}
      ...
      <div className='flex-1 p-2 space-y-1.5 overflow-y-auto'>
        {messages.map((m) => (
          <div key={m.id} className='flex items-start gap-1.5' style={{ fontSize: `${msgPx}px` }}>
            <span
              className='shrink-0 leading-tight mt-px'
              style={{
                fontSize: m.icon ? `${iconPx}px` : `${msgPx}px`,
      ...
      {userPopup && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => blockUser(userPopup.uid, userPopup.name)}
              className='w-full mt-1.5 py-1.5 text-[10px] border border-[#ff4141]/30 text-[#ff4141]/50 hover:text-[#ff4141] hover:bg-[#ff4141]/5 transition-all tracking-widest'
            >⊘ BLOQUER</button>
            <button
              onClick={() => setUserPopup(null)}
              className='w-full mt-1.5 text-[10px] text-[#00ff41]/25 hover:text-[#00ff41]/50 transition-colors'
            >Fermer</button>
          </div>
        </div>
      )}
      ...
      {/* Input */}
      <div className='border border-[#00ff41]/30 p-1.5 flex gap-1.5 items-stretch shrink-0 h-[90px]'>
        {isMuted ? (
          <div className='flex-1 h-full flex items-center justify-center text-[10px] tracking-widest border border-[#ff4141]/20 text-[#ff4141]/50'>
            {mutedUntil
              ? `MUTE · jusqu'à ${mutedUntil.toLocaleTimeString('fr-CA', { hour: '2-digit', minute: '2-digit' })}`
              : 'BANNI DU CHAT'}
          </div>
        ) : (
          <>
